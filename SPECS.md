...
class TallyBridgeConfig(BaseSettings):
    ...
    tally_encoding: str = "utf-8"         # "utf-8" or "utf-16" — must match Tally's config
...
- `post_xml` sends `Content-Type: text/xml; charset={config.tally_encoding}`
- Encode request body using `config.tally_encoding`; decode response using the same encoding
- # NOTE: Previous versions hardcoded UTF-16 response decoding. Since Phase 8e, encoding is
  configurable — Tally mirrors the request encoding in its response. Default is now UTF-8.
- Escape XML entities (e.g., `&`, `<`, `>`) in `company` and `filter_expr` before placing them in the payload
- Enforce pagination: Do not request more than `VOUCHER_BATCH_SIZE` (default 5000) at a time to prevent Tally hangs
- Use `httpx.AsyncHTTPTransport(retries=3)` for transport-level TCP retries
- # NOTE: Transport retries cover connection drops and socket errors only. Application-level
  retry (on ReadTimeout, HTTP 500, transient TallyDataError) is tracked as a future item (§analysis C6).
- Catch `httpx.ConnectError`, `httpx.ConnectTimeout`, AND `httpx.ReadTimeout` → raise `TallyConnectionError` with setup instructions
- `<EXCEPTION>` prefix in response → raise `TallyDataError`
- `<LINEERROR>` in response text or `<STATUS>-1</STATUS>` → raise `TallyDataError` with extracted error text
- # NOTE on STATUS semantics: Official TallyHelp docs say STATUS=0 means failure, but observed
  TallyPrime behavior returns STATUS=0 for empty collections (no data). STATUS=-1 indicates an
  actual error. STATUS=1 indicates success. Log STATUS=0 at debug level for visibility.
...
class TallyXMLParser:
    ...
    def parse_vouchers(self, xml: str) -> list[TallyVoucher]:
        ...
        Ensure STATUS checking is performed: <STATUS>-1</STATUS> is error (raise TallyDataError).
        <STATUS>0</STATUS> means no data (return []). <STATUS>1</STATUS> means success.
        # NOTE: Official TallyHelp docs say STATUS=0=failure, but TallyPrime returns STATUS=0
        for empty collections. See §4 implementation requirements for full discussion.
        Normalize debit/credit amounts to signed Decimals properly.
        """
...
- Before parsing lists, check for `<STATUS>`: if `0`, return `[]`. If `-1`, log error and return `[]` or raise `TallyDataError`.
...
-- Cost centre allocations (junction table — one ledger entry can split across
-- multiple cost centres with different amounts)
-- Ref: tally-database-loader/database-structure-incremental.sql trn_cost_centre
...
-- Bill allocations (for accurate outstanding/aging reports)
-- Ref: tally-database-loader/database-structure-incremental.sql trn_bill
...
class TallyCache:
    ...
    def upsert_vouchers(self, vouchers: list[TallyVoucher]) -> int:
        ...
        For each voucher:
        1. INSERT OR REPLACE the trn_voucher row (CASCADE deletes old child rows)
        2. INSERT all ledger_entries into trn_ledger_entry
        3. INSERT all inventory_entries into trn_inventory_entry
        4. INSERT all cost_centre allocations into trn_cost_centre (junction table)
        5. INSERT all bill_allocations into trn_bill
        Uses a transaction — all vouchers in the batch commit together or not at all.
        Use executemany() or DataFrame-based bulk inserts for performance.
        Create ART indexes only after bulk loading is complete.
        Ensure DECIMAL types retain full precision throughout the pipeline.
        
        # NOTE on sync_state safety: After upserting, only advance sync_state to the
        highest alter_id that was SUCCESSFULLY committed. Never advance past failed upserts
        or records will be silently skipped on the next sync cycle.
        """
    ...
    def query(self, sql: str, params: list | None = None) -> list[dict]:
        ...
        Uses a separate DuckDB read-only connection (duckdb.connect with read_only=True)
        to physically prevent write operations. Never falls back to the read-write connection.
        """
...
# Use the official MCP Python SDK v1.x (current stable release).
# FastMCP is the correct class — MCPServer is v2 pre-alpha and NOT yet stable.
# Pin: mcp>=1.2.0,<2.0.0

from mcp.server.fastmcp import FastMCP, Context
from mcp.types import CallToolResult, TextContent

# Lifespan pattern for managing TallyCache and TallyQuery:
#   @asynccontextmanager
#   async def app_lifespan(server: FastMCP):
#       cache = TallyCache(config.db_path); cache.initialize()
...
# All tools should be read-only. Use annotations when supported.
# Inject Context for access to lifespan resources:
#   @mcp.tool()
#   async def get_ledger_balance(ledger_name: str, ctx: Context) -> dict:
#       query = ctx.request_context.lifespan_context.query
#       return {"balance": str(query.get_ledger_balance(ledger_name))}

# SQL injection prevention: Use DuckDB read-only connection for query_tally_data:
#   read_conn = duckdb.connect(db_path, read_only=True)
# This physically prevents all write operations — no fragile keyword blocklists.
# # NOTE: Phase 8d removed the old is_safe_sql() blocklist approach in favor of
# this read-only connection method. Never fall back to the read-write connection.

# Error contract: EVERY tool handler must catch all exceptions and return:
# CallToolResult(is_error=True, content=[TextContent(type="text", text=str(e))])
# The server process must never crash due to a tool error.

# Run:
#   mcp.run(transport="stdio")           # Local (Claude Desktop, Cursor)
#   mcp.run(transport="streamable-http") # Remote/cloud
```
...
## §14 — Report Parsing (Balance Sheet, P&L, Trial Balance, Day Book)

### Overview

`TallyXMLParser.parse_report()` auto-detects the report type from the
`report_name` parameter and delegates to a specialist parser.  All
report responses come from `TallyConnection.fetch_report()` which uses
`TYPE=Data` in the XML request.

### Balance Sheet

- Request: `ID=Balance Sheet`
- Response tags: `<BSNAME>` groups containing `<DSPDISPNAME>`, `<BSCLOSAMT>`
  groups containing `<DSPCLDRAMTA>` / `<DSPCLCRAMTA>`
- Parsed into: `list[ReportLine]` with name, closing_debit, closing_credit
- Sections: Assets (positive balance) and Liabilities (negative balance)

### Profit & Loss

- Request: `ID=Profit & Loss`
- Response tags: `<PLNAME>` / `<PLCLOSAMT>` (same structure as BS)
- Parsed into: `list[ReportLine]`
- Sections: Income (credit) and Expense (debit)

### Trial Balance (report)

- Request: `ID=Trial Balance`
- Response tags: `<DSPACCNAME>` with `<DSPDISPNAME>`, `<DSPACCINFO>` with
  `<DSPCLDRAMTA>` / `<DSPCLCRAMTA>`
- Parsed into: `list[TrialBalanceLine]` with ledger, group, closing_debit, closing_credit

### Day Book

- Request: `ID=Day Book`
- Response: Contains `<TALLYMESSAGE>` with `<VOUCHER>` elements
- Parsed into: `list[dict]` with date, voucher_type, voucher_number, narration, guid

### TallyReport model

```python
class TallyReport(BaseModel):
    report_type: Literal["Balance Sheet", "Profit & Loss", "Trial Balance", "Day Book", "Unknown"]
    from_date: date | None
    to_date: date | None
    lines: list[ReportLine]        # BS and P&L
    trial_balance: list[TrialBalanceLine]  # TB
    vouchers: list[dict]           # Day Book
```

### fetch_report() parse mode

`fetch_report(parse=True)` returns a structured `TallyReport` instead of
raw XML.  The `parse=False` (default) returns the raw XML string for
backward compatibility.

## §15 — Deletion Tracking

### Problem

Records deleted in Tally persist forever in the DuckDB cache because
`sync_all()` only fetches new/modified records via AlterID ranges.

### Solution: GUID diff during full_sync()

1. During `full_sync()`, after re-syncing all entities, call
   `detect_deletions()` for each master entity type (not vouchers)
2. Fetch all GUIDs from Tally using `export_collection()` with `fields=["GUID"]`
3. Compare against `cache.get_cached_guids(entity_type)` — any GUID in
   cache but missing from Tally is considered deleted
4. Call `cache.delete_records_by_guid(entity_type, deleted_guids)` to remove
5. For vouchers, cascade-delete child tables: `trn_ledger_entry`,
   `trn_inventory_entry`, `trn_cost_centre`, `trn_bill`
6. Log deletion counts at warning level

### API

```python
# TallySyncEngine
async def detect_deletions(self, entity_types: list[str] | None = None) -> dict[str, int]
...
### Why not for vouchers?

Vouchers use `ISCANCELLED` and `ISVOID` fields rather than hard deletion.
Cancelled vouchers are handled by the parser setting these flags.
...
Our MCP tools never filtered by company.

### Solution

Add optional `company: str | None = None` parameter to all MCP tools
(except `query_tally_data` which uses raw SQL).  The parameter is
accepted but not yet enforced in queries — it provides the API surface
for future filtering.

### Affected tools

All 15 tools except `query_tally_data`:
`get_tally_digest`, `get_ledger_balance`, `get_receivables`,
`get_party_outstanding`, `get_sales_summary`, `get_gst_summary`,
`search_tally`, `get_sync_status`, `get_low_stock`, `get_stock_aging`,
`get_cost_center_summary`, `get_balance_sheet`, `get_profit_loss`,
`get_ledger_account`, `get_stock_item_account`

## §17 — Version Gating Integration

### Problem

`detect_tally_version()` and `capabilities()` exist in `version.py` but
aren't called from the connection flow.

### Solution

1. `TallyConnection.detect_version()` — convenience method that calls
   `detect_tally_version()` and logs the capability set
2. `TallySyncEngine.sync_all()` calls `connection.detect_version()` on
   first sync
3. The detected version is cached on `connection._detected_version`
4. Key capability gates:
   - `allledger_entries`: ERP 9 doesn't have ALLLEDGERENTRIES.LIST
   - `json_api`: Only TallyPrime 7.0+
   - `base64_encoding`: Only TallyPrime 7.0+
   - `tally_drive`: Only TallyPrime 7.0+

### Parser version awareness

`TallyXMLParser._get_ledger_entry_elements()` already falls back from
`ALLLEDGERENTRIES.LIST` to `LEDGERENTRIES.LIST` for ERP 9 compatibility.
Same for inventory entries.

## §18 — Missing MCP Tools

### Added tools (10f)

| Tool | Description |
|------|-------------|
| `get_balance_sheet(to_date, company?)` | Balance sheet grouped by assets/liabilities |
| `get_profit_loss(from_date, to_date, company?)` | P&L grouped by income/expense |
| `get_ledger_account(ledger_name, from_date, to_date, company?)` | Voucher-level GL |
| `get_stock_item_account(item_name, from_date, to_date, company?)` | Quantity movements |

### Query methods added

- `TallyQuery.get_balance_sheet(to_date)` → queries mst_ledger closing balances
- `TallyQuery.get_profit_loss(from_date, to_date)` → queries trn_ledger_entry by parent_group
- `TallyQuery.get_ledger_account(ledger_name, from_date, to_date)` → voucher-level entries
- `TallyQuery.get_stock_item_account(item_name, from_date, to_date)` → inventory movements
...
- TallyPrime uses `<ALLLEDGERENTRIES.LIST>` for voucher ledger entries
- Tally.ERP 9 uses `<LEDGERENTRIES.LIST>`
- Parser fallback: if ALLLEDGERENTRIES.LIST returns nothing, try LEDGERENTRIES.LIST

### ALLINVENTORYENTRIES.LIST vs INVENTORYENTRIES.LIST

- Same pattern for inventory entries
- Parser fallback implemented in `_get_inventory_entry_elements()`

### ACCOUNTINGALLOCATIONS.LIST

- Present in some Tally versions for cost centre allocation details
- Parser attempts to extract cost centre info from these elements

### Impact on version gating

- ERP 9: `capabilities()["allledger_entries"]` returns `False`
- TallyPrime 1+: `capabilities()["allledger_entries"]` returns `True`
- Parser handles both transparently via fallback

## §20 — Real Tally Validation Testing Guidance

### Setting up a test environment

1. Install TallyPrime (free educational edition available)
2. Create a test company with sample data
3. Enable the HTTP server: Gateway of Tally → F12 → Advanced → Enable ODBC Server = Yes
4. Default port: 9000

### Key test scenarios

1. **Sync**: Run `tallybridge sync` and verify all entity types sync
2. **Incremental sync**: Create a new ledger in Tally, re-sync, verify only new records fetched
3. **Version detection**: Verify `detect_tally_version()` returns correct product
...
6. **ERP 9 compatibility**: Test with Tally.ERP 9 to verify LEDGERENTRIES.LIST fallback

### Validation checklist

- [ ] All 17 MCP tools return data
- [ ] Version detection shows correct TallyProduct
- [ ] Balance Sheet and P&L report parsing works
- [ ] Deletion detection removes orphans
- [ ] ERP 9 LEDGERENTRIES.LIST fallback works
- [ ] Company filtering passes through to queries
- [ ] Authentication works for HTTP transport
